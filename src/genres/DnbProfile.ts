 ...
 */
export interface DnbProfileWeights {
  velocity: number;
  transients: number;
  subEnergy: number;
  tempo: number;
...
export const DNB_WEIGHTS: DnbProfileWeights = {
  velocity: 0.35,
  transients: 0.25,
  subEnergy: 0.2,
  tempo: 0.2,
...
export function dnbTempoTendency(bpm: number): number {
  if (bpm <= 0) return 0;
...
export function scoreDnb(music: MusicState, weights: DnbProfileWeights = DNB_WEIGHTS): number {
  ...
  const score =
    ...
    weights.tempo * dnbTempoTendency(music.bpm);
